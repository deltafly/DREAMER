...
# NOTE: this used to build and package a `mini-services/` directory — two
# standalone servers with no authentication of their own, one of which pinned
# every request to workspace 1 and so bypassed the app's tenant checks.
# They were removed: the authenticated /api/mcp route replaces them.
