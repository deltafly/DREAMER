...
| Fájl | Export | Leírás |
|------|--------|--------|
| `llm-client.ts` | `complete()`, `resolveProvider()`, `resolveModel()`, `describeLLM()` | Provider-agnosztikus belépési pont. Két adapter: `anthropic` (hivatalos SDK) és `openai` (bármely OpenAI-kompatibilis `/chat/completions`). A második szándékosan a széles kapu: ugyanaz az alak elér OpenAI-t, OpenRoutert (pl. `z-ai/glm-5.2`), Groqot, vLLM-et és lokális Ollamát — így „új szolgáltató" általában egy base URL + modellnév, nem kód. Az adapter nyeli le az eltéréseket: a `temperature` NEM megy a Claude-nak (400-zal utasítaná el), az `effort` hintet a nem ismerő providerek eldobják, a Claude `refusal` stop reason-je `LLMUnavailableError`-rá alakul. Konfigurálatlan állapotban beszédes hibát dob. |
